...
class GiniDecisionTreeClassifier:
    def __init__(
        ...
        min_gini_decrease：
            基尼指数表示随机抽取两个样本不属于同一类别的概率
            基尼指数越小表示数据集纯度越高
            因此能使基尼指数变小的属性才是可以考虑划分的属性
        """
    ...
    def _find_best_split(
        ...
        for feature_index in range(n_features):
            ...
            for threshold in thresholds:
                ...
                # 选择加权基尼指数最小的划分
                if children_gini < best_gini:
                    best_gini = children_gini
                    ...
                    best_split = {
                        ...
                        "split_gini": float(children_gini),
                        "gini_decrease": float(parent_gini - children_gini),
                    }
    ...
    def _build_tree(
        ...
        # 停止条件5：基尼指数下降太少
        if best_split["gini_decrease"] <= self.min_gini_decrease:
            return TreeNode(
                prediction=majority_class
            )
        ...
        return TreeNode(
            ...
            right=right_tree,
            prediction=majority_class,
            gini_decrease=best_split["gini_decrease"],
            split_gini=best_split["split_gini"]
        )
...
if __name__ == "__main__":
    ...
    # 注意这里参数的设置
    # 没有设置树的最大深度，同时两个样本也允许继续划分
    # 叶节点只包含一个样本也合法
    # 可能会有过拟合的风险
    model = GiniDecisionTreeClassifier(
        max_depth=None,
        min_samples_split=2,
        min_samples_leaf=1,
    )
